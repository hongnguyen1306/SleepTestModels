...
class DeepFeatureNet(object):
    ...
    def _conv1d_layer(self, input_var, filter_size, n_filters, stride, wd=0):
        ...
        if isinstance(input_shape, int):
            n_in_filters = input_shape
        else:
            n_in_filters = input_shape[3]
        name = "l{}_conv".format(self.layer_idx)
        with tf.compat.v1.variable_scope(name) as scope:
            output = conv_1d(name="conv1d", input_var=input_var, filter_shape=[
                             filter_size, 1, n_in_filters, n_filters], stride=stride, bias=None, wd=wd)

            # # MONITORING
            # self.monitor_vars.append(("{}_before_bn".format(name), output))

            output = batch_norm_new(
                name="bn", input_var=output, is_train=self.is_train)
    ...
    def build_model(self, input_var):
        ...
        network = self._conv1d_layer(
            input_var=input_var, filter_size=50, n_filters=64, stride=6, wd=1e-3)

        # Max pooling
        name = "l{}_pool".format(self.layer_idx)
        network = max_pool_1d(
            name=name, input_var=network, pool_size=8, stride=8)
        ...
        # Convolution
        network = self._conv1d_layer(
            input_var=network, filter_size=8, n_filters=128, stride=1)
        network = self._conv1d_layer(
            input_var=network, filter_size=8, n_filters=128, stride=1)
        network = self._conv1d_layer(
            input_var=network, filter_size=8, n_filters=128, stride=1)
        ...
        name = "l{}_pool".format(self.layer_idx)
        network = max_pool_1d(
            name=name, input_var=network, pool_size=4, stride=4)
        self.activations.append((name, network))
        self.layer_idx += 1
        ...
        network = self._conv1d_layer(
            input_var=input_var, filter_size=400, n_filters=64, stride=50)
        ...
        # Convolution
        network = self._conv1d_layer(
            input_var=network, filter_size=6, n_filters=128, stride=1)
        network = self._conv1d_layer(
            input_var=network, filter_size=6, n_filters=128, stride=1)
        network = self._conv1d_layer(
            input_var=network, filter_size=6, n_filters=128, stride=1)
        ...
        name = "l{}_pool".format(self.layer_idx)
        network = max_pool_1d(
            name=name, input_var=network, pool_size=2, stride=2)
        self.activations.append((name, network))
        self.layer_idx += 1
    ...
    def init_ops(self):
        ...
        with tf.compat.v1.variable_scope(self.name) as scope:
            ...
            network = fc(name=name, input_var=network,
                         n_hiddens=self.n_classes, bias=0.0, wd=0)
            self.activations.append((name, network))
            self.layer_idx += 1
...
class DeepSleepNetNoLabels(DeepFeatureNet):
    ...
    def init_ops(self):
        ...
        with tf.compat.v1.variable_scope(self.name) as scope:
            ...
            # Total loss
            self.loss_op = tf.constant(0, dtype=tf.float32)
