...
def delete_files_async(folder_path, extension):
    thread = threading.Thread(
        target=delete_files_with_extension, args=(folder_path, extension))
    thread.start()
...
def predict():
    ...
    test_dl = data_generator(
        str(os.path.join(base_path, "data/test_data.pt")), labels=False)

    total_loss, total_acc_TS, outs_TS, trgs = load_model_TCC(
        test_dl, base_path, method='TS', act_func='GELU', labels=False)
    total_loss, total_acc_CA, outs_CA, trgs = load_model_TCC(
        test_dl, base_path, method='CA', act_func='GELU', labels=False)
    acc, f1_score, outs_tiny = load_model_Tiny(
        test_npz, base_path, act_func='ReLU', labels=False)
...
def evaluate():
    ...
    test_pt = data_generator(os.path.join(
        base_path, "data/test_data.pt"), labels=True)

    # print("\n*****    ReLU    ******")
    total_loss, relu_acc_TS, outs_TS, true_labels = load_model_TCC(
        test_pt, base_path, method='TS', act_func='ReLU')
    total_loss, relu_acc_CA, outs_CA, trgs = load_model_TCC(
        test_pt, base_path, method='CA', act_func='ReLU')

    # print("\n*****    GELU    ******")
    total_loss, gelu_acc_TS, outs_TS_G, trgs = load_model_TCC(
        test_pt, base_path, method='TS', act_func='GELU')
    total_loss, gelu_acc_CA, outs_CA_G, trgs = load_model_TCC(
        test_pt, base_path, method='CA', act_func='GELU')

    relu_acc_tiny, f1_tiny_relu, outs_tiny_ReLU = load_model_Tiny(
        test_npz, base_path, act_func='ReLU', labels=True)
    gelu_acc_tiny, f1_tiny_gelu, outs_tiny_GELU = load_model_Tiny(
        test_npz, base_path, act_func='GELU', labels=True)
